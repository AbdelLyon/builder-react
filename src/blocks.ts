...
export function loadBlocks(editor: Editor, opts: Required<PluginOptions>) {
  ...
  .${stylePrefix}cell70 {
   ...
   const minDim = 1;
   const currentUnit = 1;
   const resizerBtm: ResizerOptions = {
      tl: 0,
      tc: 0,
      tr: 0,
      cl: 0,
   ...
   const resizerRight: ResizerOptions = {
      ...resizerBtm,
      cr: 1,
      bc: 0,
      currentUnit,
   ...
   const colAttr: Record<string, string | ResizerOptions | string[]> = {
      class: clsCell,
   ...
   const attrsToString = (attrs: Record<string, string | string[] | object>) => {
      const result = [];
